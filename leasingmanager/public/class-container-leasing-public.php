 ...
 * @subpackage Container_Leasing/public
 */
 ...
 * @subpackage Container_Leasing/public
 */
class Container_Leasing_Public {
    ...
    public function enqueue_styles() {
        wp_enqueue_style($this->plugin_name, plugin_dir_url(__FILE__) . 'css/container-leasing-public.css', array(), $this->version, 'all');
    ...
    public function enqueue_scripts() {
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/container-leasing-public.js', array('jquery'), $this->version, false);
    ...
    public function add_custom_templates($templates) {
        $templates['template-insights.php'] = __('Container Insights', 'container-leasing');
        $templates['template-container-management.php'] = __('Container Management', 'container-leasing');
        $templates['template-invoices.php'] = __('Container Invoices', 'container-leasing');
        $templates['template-email-management.php'] = __('Email Management', 'container-leasing');
        $templates['template-dashboard.php'] = __('Container Dashboard', 'container-leasing');
    ...
    public function handle_save_container() {
        ...
        check_ajax_referer('container_leasing_nonce', 'nonce');
        
        // Check if user is logged in
        if (!is_user_logged_in()) {
            wp_send_json_error(__('You must be logged in to save a container.', 'container-leasing'));
        }
        ...
        wp_send_json_success(array(
            ...
            'message' => __('Container saved successfully.', 'container-leasing')
        ));
    }
    ...
    public function handle_load_insights() {
        ...
        // Check if user is logged in
        if (!is_user_logged_in()) {
            wp_send_json_error(__('You must be logged in to view insights.', 'container-leasing'));
        }
    ...
    public function handle_generate_invoice() {
        ...
        // Check if user is logged in
        if (!is_user_logged_in()) {
            wp_send_json_error(__('You must be logged in to generate an invoice.', 'container-leasing'));
        }
        ...
        if ($contract_id <= 0) {
            wp_send_json_error(__('Invalid contract ID.', 'container-leasing'));
        }
        ...
        if (!$contract) {
            wp_send_json_error(__('Contract not found.', 'container-leasing'));
        }
        
        // Check if user owns this contract
        if ($contract->user_id != get_current_user_id() && !current_user_can('administrator')) {
            wp_send_json_error(__('You do not have permission to access this contract.', 'container-leasing'));
        }
    ...
    public function handle_upload_csv() {
        ...
        wp_send_json_success(array(
            'message' => sprintf(
                __('CSV import completed. %d containers imported successfully.', 'container-leasing'),
                $result
            )
        ));
    }
