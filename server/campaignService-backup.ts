...
export class CampaignService {
  ...
  private static async getAudienceRecipients(audience: string, campaignId?: number): Promise<Array<{ userId: number; email: string; firstName?: string }>> {
    ...
    switch (audience) {
      ...
      case 'subscribers':
        query = db
          .select({
            userId: sql`0`.as('userId'), // Default for email-only subscribers
  ...
  static async addRecipientsToCampaign(campaignId: number, emails: string[]): Promise<void> {
    const recipientData: InsertCampaignRecipient[] = emails.map(email => ({
      campaignId,
      userId: 0, // Required field for manually added recipients
      email,
      status: 'pending'
    }));
  ...
  static async removeRecipientsFromCampaign(campaignId: number, emails: string[]): Promise<void> {
    await db
      ...
      .where(and(
        ...
        sql`${campaignRecipients.email} = ANY(${emails})`
      ));
  }
  ...
  static async getCampaignAnalytics(campaignId: number): Promise<{
    ...
    analytics.forEach(stat => {
      switch (stat.eventType) {
        case 'sent':
          stats.sent = Number(stat.count);
          break;
        case 'delivered':
          stats.delivered = Number(stat.count);
          break;
        case 'opened':
          stats.opened = Number(stat.count);
          break;
        case 'clicked':
          stats.clicked = Number(stat.count);
          break;
        case 'bounced':
          stats.bounced = Number(stat.count);
          break;
        case 'unsubscribed':
          stats.unsubscribed = Number(stat.count);
          break;
      }
    });
  ...
  // Add recipients to campaign (bulk email management)
  static async addRecipientsToCampaign(campaignId: number, emails: string[]): Promise<void> {
    const campaign = await this.getCampaignById(campaignId);
    if (!campaign) {
      throw new Error('Campaign not found');
    ...
    try {
      ...
      await db
        .update(emailCampaigns)
        .set({ 
          recipientCount: sql`${emailCampaigns.recipientCount} + ${validEmails.length}`,
          updatedAt: sql`NOW()`
        })
        .where(eq(emailCampaigns.id, campaignId));
      ...
      console.log('Some recipients may already exist for this campaign');
    }
  }
  ...
  static async removeRecipientsFromCampaign(campaignId: number, emails: string[]): Promise<void> {
    ...
    await db
      .update(emailCampaigns)
      .set({ 
        recipientCount: remainingCount[0]?.count || 0,
        updatedAt: sql`NOW()`
      })
      .where(eq(emailCampaigns.id, campaignId));
  }

  // Parse bulk email import (CSV, text formats)
  static parseBulkEmailImport(content: string): string[] {
    const emails: string[] = [];
    ...
    for (const line of lines) {
      ...
      // Extract emails from CSV format
