...
async function importContainersOnly() {
  try {
    await client.connect();
    console.log('Connected to database');
    
    // Get existing depot mapping
    const depotResult = await client.query('SELECT id, depot_name FROM depot_locations');
    const depotsMap = new Map();
    depotResult.rows.forEach(row => {
      depotsMap.set(row.depot_name, row.id);
    });
    console.log(`Found ${depotsMap.size} existing depots`);
    ...
    // Clear existing containers and import all from CSV
    ...
    for (let i = 0; i < rows.length; i += batchSize) {
      ...
      for (const row of batch) {
        ...
        if (!depotId) {
          console.log(`Skipping container for unknown depot: ${row.depot_name}`);
          continue;
        }
        ...
        try {
          ...
          console.error(`Error creating container ${row.sku}:`, containerError.message);
        }
      }
      
      if (containerCount % 500 === 0) {
        console.log(`Imported ${containerCount} containers...`);
      }
    }
    
    console.log(`Successfully imported ${containerCount} containers`);
    await client.end();
