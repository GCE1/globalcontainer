...
wss.on('connection', (ws) => {
  ...
  ws.send(JSON.stringify({
    type: 'connection',
    message: 'Connected to Container Wholesale WebSocket server'
  }));
...
app.get("/api/wholesale-locations", async (req, res) => {
  try {
    ...
    console.error('Error getting wholesale locations:', error);
    res.status(500).json({ error: 'Failed to get wholesale locations' });
  }
});
