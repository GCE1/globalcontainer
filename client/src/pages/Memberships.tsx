import { useState } from "react";
import { Link, useLocation } from "wouter";
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import { useUserRoles } from "@/hooks/useUserRoles";
import { useAuth } from "@/hooks/useAuth";

import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Check, ArrowRight } from "lucide-react";
// Optimized hero image
const membershipHeroImage = "/optimized_assets/Membership-hero-image_1749589356481-optimized.webp";

export default function Memberships() {
  const [, setLocation] = useLocation();
  const { user, isLoading: authLoading } = useAuth();
  const { data: userRoles, isLoading: rolesLoading, error: rolesError } = useUserRoles();
  
  console.log('Memberships Debug:', {
    user,
    authLoading,
    userRoles,
    rolesLoading,
    rolesError,
    token: localStorage.getItem('authToken') ? 'exists' : 'missing'
  });
  
  const handleNavigation = (path: string) => {
    setLocation(path);
    setTimeout(() => {
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }, 100);
  };
  
  const hasRole = (roleType: string) => {
    // If still loading, return false
    if (authLoading || !user) return false;
    
    // Primary check: use roles from user object (populated during login)
    if (user.roles && Array.isArray(user.roles)) {
      const hasActiveRole = user.roles.some(role => 
        role.role_type === roleType && role.subscription_status === 'active'
      );
      
      console.log(`Role check for ${roleType}:`, {
        userRoles: user.roles,
        hasActiveRole,
        matchingRoles: user.roles.filter(role => role.role_type === roleType)
      });
      
      return hasActiveRole;
    }
    
    // Secondary check: use userRoles from the API hook
    if (!rolesLoading && userRoles && Array.isArray(userRoles)) {
      const hasActiveRole = userRoles.some(role => 
        role.role_type === roleType && role.subscription_status === 'active'
      );
      
      console.log(`Role check via API for ${roleType}:`, {
        userRoles,
        hasActiveRole,
        matchingRoles: userRoles.filter(role => role.role_type === roleType)
      });
      
      return hasActiveRole;
    }
    
    // Fallback: use legacy subscription data from user object
    if (user && user.subscriptionTier === roleType && user.subscriptionStatus === 'active') {
      console.log(`Using legacy subscription data for ${roleType}:`, {
        subscriptionTier: user.subscriptionTier,
        subscriptionStatus: user.subscriptionStatus
      });
      return true;
    }
    
    console.log(`No role data available for ${roleType}`, { user, userRoles });
    return false;
  };
  
  // Show login message if user is not authenticated
  if (!authLoading && !user) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-teal-50 to-blue-50">
        <Navbar />
        <div className="container mx-auto px-4 py-16 text-center">
          <h2 className="text-3xl font-bold text-[#001937] mb-4">Access Your Membership</h2>
          <p className="text-gray-600 mb-8">Please log in to view your membership status and access your purchased subscriptions.</p>
          <Button 
            className="bg-[#001937] hover:bg-[#33d2b9] text-white font-semibold px-8 py-3 rounded-md transition-all duration-300"
            onClick={() => handleNavigation('/login')}
          >
            Log In
          </Button>
        </div>
        <Footer />
      </div>
    );
  }
  
  const serviceBlocks = [
    {
      title: "Insights",
      subtitle: "Analytics & Tracking",
      content: (
        <div className="flex flex-col h-full">
          <div className="mb-4">
            <span className="text-gray-400 line-through text-sm">$99</span>
            <span className="text-2xl font-bold text-[#001937] ml-2">$49</span>
            <span className="text-gray-600 text-sm ml-1">month</span>
          </div>
          <ul className="text-gray-700 text-xs space-y-2 flex-grow">
            <li>★ Track All Container Purchases by Location</li>
            <li>★ Track All Container Leases by Location</li>
            <li>★ Analytics Dashboard with Performance Metrics</li>
            <li>★ Real-Time Container Route Tracking</li>
            <li>★ Monthly Insights Reports & Cost Analysis</li>
          </ul>
          {hasRole('insights') ? (
            <Button 
              className="bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-md transition-all duration-300 shadow-md hover:shadow-lg w-full"
              onClick={() => handleNavigation('/insights-analytics')}
            >
              Access Granted
            </Button>
          ) : (
            <Button 
              className="bg-[#001937] hover:bg-[#33d2b9] text-white font-semibold px-6 py-3 rounded-md transition-all duration-300 shadow-md hover:shadow-lg w-full"
              onClick={() => handleNavigation('/payment?tier=insights&price=49')}
            >
              Subscribe Now
            </Button>
          )}
        </div>
      )
    },
    {
      title: "Professional",
      subtitle: "Wholesale",
      content: (
        <div className="flex flex-col h-full">
          <div className="mb-4">
            <span className="text-gray-400 line-through text-sm">$299</span>
            <span className="text-2xl font-bold text-[#001937] ml-2">$199</span>
            <span className="text-gray-600 text-sm ml-1">month</span>
          </div>
          <ul className="text-gray-700 text-xs space-y-2 flex-grow">
            <li>★ Wholesale Rates Throughout 89 Countries</li>
            <li>★ CRM Dashboard Optimize Sales & Margins</li>
            <li>★ Real-Time Container Route Tracking</li>
            <li>★ Depot Directory</li>
            <li>★ Buy Now Pay Later Options Allowing for customer Reassurances</li>
          </ul>
          {hasRole('pro') ? (
            <Button 
              className="bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-md transition-all duration-300 shadow-md hover:shadow-lg w-full"
              onClick={() => handleNavigation('/wholesale-manager')}
            >
              Access Granted
            </Button>
          ) : (
            <Button 
              className="bg-[#001937] hover:bg-[#33d2b9] text-white font-semibold px-6 py-3 rounded-md transition-all duration-300 shadow-md hover:shadow-lg w-full"
              onClick={() => handleNavigation('/payment?tier=pro&price=199')}
            >
              Subscribe Now
            </Button>
          )}
        </div>
      )
    },
    {
      title: "Expert",
      subtitle: "Leasing",
      content: (
        <div className="flex flex-col h-full">
          <div className="mb-4">
            <span className="text-gray-400 line-through text-sm">$249</span>
            <span className="text-2xl font-bold text-[#001937] ml-2">$149</span>
            <span className="text-gray-600 text-sm ml-1">month</span>
          </div>
          <ul className="text-gray-700 text-xs space-y-2 flex-grow">
            <li>★ Leasing Throughout 89 Countries</li>
            <li>★ Utilize SOC Rates</li>
            <li>★ Real-Time Container Route Tracking</li>
            <li>★ Foreign Trade Depots</li>
            <li>★ Don't pay VAT & Duty Fees before products are sold or relocated</li>
          </ul>
          {hasRole('expert') ? (
            <Button 
              className="bg-green-600 hover:bg-green-700 text-white font-semibold px-6 py-3 rounded-md transition-all duration-300 shadow-md hover:shadow-lg w-full"
              onClick={() => handleNavigation('/leasing-manager')}
            >
              Access Granted
            </Button>
          ) : (
            <Button 
              className="bg-[#001937] hover:bg-[#33d2b9] text-white font-semibold px-6 py-3 rounded-md transition-all duration-300 shadow-md hover:shadow-lg w-full"
              onClick={() => handleNavigation('/payment?tier=expert&price=149')}
            >
              Subscribe Now
            </Button>
          )}
        </div>
      )
    }
  ];

  const memberships = [
    {
      id: "insights",
      name: "Insights",
      price: "49",
      description: "Essential container market insights and basic analytics",
      features: [
        "Basic Market Analytics",
        "Container Availability Data",
        "Price Trend Reports",
        "Monthly Market Insights",
        "Email Support",
        "Standard Dashboard Access",
        "Export Basic Reports"
      ],
      popular: false
    },
    {
      id: "professional",
      name: "Professional",
      price: "199",
      description: "Advanced tools for professional container trading operations",
      features: [
        "Full Analytics Dashboard",
        "Real-time Market Data",
        "Advanced Search Filters",
        "Predictive Analytics",
        "API Access",
        "Priority Customer Support",
        "Custom Reports",
        "Inventory Management Tools",
        "Multi-user Access"
      ],
      popular: true
    },
    {
      id: "expert",
      name: "Expert",
      price: "149",
      description: "Complete enterprise solution with premium features",
      features: [
        "Intelligent location search",
        "Nearest depot optimization",
        "Global postal code support",
        "Real-time distance calculation",
        "Enterprise support",
        "Custom logistics solutions"
      ],
      popular: false
    }
  ];

  return (
    <div className="min-h-screen flex flex-col">
      <Navbar />
      <main className="flex-grow">
        {/* Header Section */}
        <section 
          className="relative text-white px-6 lg:px-8 bg-cover bg-center bg-no-repeat flex items-center"
          style={{
            backgroundImage: `url(${membershipHeroImage})`,
            minHeight: '375px'
          }}
        >
          {/* Dark overlay for better text readability */}
          <div className="absolute inset-0 bg-black bg-opacity-40"></div>
          
          <div className="relative z-10 max-w-4xl mx-auto text-center w-full">
            <h1 className="text-4xl md:text-5xl font-bold mb-6 text-white drop-shadow-lg">
              Choose Your Membership
            </h1>
            <p className="text-xl text-white drop-shadow-md">
              Unlock powerful platform features with our specialized memberships
            </p>
          </div>
        </section>

        {/* Main Content - Matching ServiceBlocksSection Design */}
        <section className="pt-8 pb-3 bg-white" style={{marginBottom: '5px'}}>
          <div className="container mx-auto px-4">
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-[#001937] mb-2">Become a Member - Exclusive package options</h2>
              <p className="text-lg text-gray-600">Join the Global Container Exchange platform Today!</p>
            </div>
            
            {/* New integrated layout with image and packages */}
            <div className="flex flex-col lg:flex-row gap-8 items-stretch">
              {/* Left side - Membership packages */}
              <div className="flex-1 lg:w-2/3 flex flex-col">
                <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6 mb-6">
                  {serviceBlocks.map((block, index) => (
                    <div key={index} className="bg-gradient-to-br from-[#001937] to-[#4a90e2] rounded-lg p-6 shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col relative">
                      <div className="absolute top-6 right-6 z-10">
                        <img 
                          src="/attached_assets/PayPal-Buynow-pay-later.png"
                          alt="PayPal Buy Now Pay Later"
                          className="h-9 w-auto border border-white/30 rounded"
                          style={{ 
                            filter: 'contrast(1.15) brightness(1.08) saturate(1.05)',
                            imageRendering: '-webkit-optimize-contrast'
                          }}
                          onError={(e) => {
                            console.log('PayPal logo failed to load:', e.currentTarget.src);
                          }}
                          onLoad={() => {
                            console.log('PayPal logo loaded successfully');
                          }}
                        />
                      </div>
                      <div className="mb-4 pr-20">
                        <div className="text-left">
                          <h3 className="text-xl font-bold text-white mb-2">
                            {block.title}
                          </h3>
                          <h4 className="text-sm font-semibold text-white/90 whitespace-nowrap">
                            {block.subtitle}
                          </h4>
                        </div>
                      </div>
                      
                      <div className="bg-white rounded-lg p-4 flex-1 flex flex-col shadow-inner">
                        <div className="text-gray-800 h-full">
                          {block.content}
                        </div>
                      </div>
                    </div>
                  ))}
                </div>
                
                {/* Why choose Global Container Exchange? section */}
                <div className="flex-1 bg-gray-100 rounded-lg p-6 shadow-md flex items-center">
                  <div className="text-center w-full">
                    <h3 className="text-lg font-semibold text-[#001937] mb-3">
                      Why Choose Global Container Exchange?
                    </h3>
                    <div className="grid md:grid-cols-3 gap-4 text-sm text-gray-700">
                      <div className="flex flex-col items-center">
                        <div className="text-[#33d2b9] text-xl mb-2">🌍</div>
                        <p className="font-medium text-blue-600">Global Network</p>
                        <p>Access to 89 countries with established depot partnerships worldwide</p>
                      </div>
                      <div className="flex flex-col items-center">
                        <div className="text-[#33d2b9] text-xl mb-2">💰</div>
                        <p className="font-medium text-blue-600">Best Pricing</p>
                        <p>Wholesale rates and SOC options that save you money on every transaction</p>
                      </div>
                      <div className="flex flex-col items-center">
                        <div className="text-[#33d2b9] text-xl mb-2">🔒</div>
                        <p className="font-medium text-blue-600">Secure & Trusted</p>
                        <p>Bank-level security and verified partnerships with major shipping lines worldwide</p>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              
              {/* Right side - Expert Support card */}
              <div className="flex-1 lg:w-1/3 lg:max-w-md">
                <div className="bg-gradient-to-br from-[#001937] to-[#4a90e2] rounded-lg p-6 shadow-lg h-full">
                  <div className="text-center mb-6">
                    <h3 className="text-2xl font-bold text-white mb-3">Expert Support</h3>
                    <p className="text-white/90 text-sm leading-relaxed">
                      Our dedicated team of container logistics experts is here to help you succeed. 
                      Get personalized guidance for your shipping needs.
                    </p>
                  </div>
                  
                  <div className="relative mb-6">
                    <img 
                      src="/images/customer-service.jpg" 
                      alt="Global Container Exchange Expert Support" 
                      className="w-full h-64 object-cover rounded-lg shadow-md"
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent rounded-lg"></div>
                  </div>
                  
                  <div className="space-y-3 text-white/90 text-sm">
                    <div className="flex items-center">
                      <div className="w-2 h-2 bg-[#33d2b9] rounded-full mr-3"></div>
                      <span>24/7 Customer Support</span>
                    </div>
                    <div className="flex items-center">
                      <div className="w-2 h-2 bg-[#33d2b9] rounded-full mr-3"></div>
                      <span>Personalized Consultation</span>
                    </div>
                    <div className="flex items-center">
                      <div className="w-2 h-2 bg-[#33d2b9] rounded-full mr-3"></div>
                      <span>Global Logistics Expertise</span>
                    </div>
                  </div>
                  
                  <Button 
                    className="w-full mt-6 bg-white text-[#001937] hover:bg-gray-100 font-semibold"
                    onClick={() => handleNavigation('/contact-us')}
                  >
                    Get Expert Help
                  </Button>
                </div>
              </div>
            </div>
          </div>
        </section>


      </main>
      <Footer />
    </div>
  );
}