...
export const useDistanceCalculation = () => {
  ...
  const findNearestDepot = useCallback(async (
    containers: any[],
    userLat: number,
    userLng: number
  ): Promise<{ depot: string; distance: number; containers: ContainerWithDistance[] } | null> => {
    try {
      // Group containers by depot
      const depotGroups = containers.reduce((groups, container) => {
        const depotKey = container.depot_name;
        if (!groups[depotKey]) {
          groups[depotKey] = [];
        }
        groups[depotKey].push(container);
        return groups;
      }, {} as Record<string, any[]>);

      // Calculate distance to each depot (using first container coordinates)
      const depotDistances = await Promise.all(
        Object.entries(depotGroups).map(async ([depotName, depotContainers]) => {
          const firstContainer = depotContainers[0];
          const distance = await calculateDistance(
            userLat,
            userLng,
      ...
      // Find nearest depot
      const nearest = depotDistances.reduce((closest, current) => 
        current.distance < closest.distance ? current : closest
      );

      // Add distance to all containers from nearest depot
      const containersWithDistance = await calculateContainerDistances(
        nearest.containers,
        userLat,
        userLng
      );
      ...
      setError(`Failed to find nearest depot: ${err.message}`);
      return null;
    }
